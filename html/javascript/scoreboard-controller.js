...
var ScoreBoardNodeHelper_select = Class.create(ScoreBoardNodeHelper, {
  initialize: function($super, node, element) {
    ...
    var optionTextElementPrefix = "OptionTextElement:";
    var optionTextElementText = $w(this.element.className).find(function (n) { return n.startsWith(optionTextElementPrefix); });
    if (optionTextElementText)
      this.optionTextElement = optionTextElementText.sub(optionTextElementPrefix, "");
  ...
  createOption: function(target) {
    ...
    if (this.optionTextElement) {
      o.store("TextObserver", function(event) { this.text = event.memo.value; }.bind(o));
      o.text = target.get(this.optionTextElement).getContent();
      target.get(this.optionTextElement).observe("content:change", o.retrieve("TextObserver"));
    } else {
      o.text = target.getId();
    }
  ...
  removeOption: function(target) {
    $A(this.element.options).each(function (option) {
      if (o.retrieve("TargetId") == target.getId()) {
        if (this.optionTextElement)
          target.get(this.optionTextElement).stopObserving(o.retrieve("TextObserver"));
        option.remove();
      }
    });
    this.updateSelectedIndex();
