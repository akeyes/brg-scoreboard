...
//FIXME - add hashtable here, or in loadfromfile class, to handle multiple saves instead of a single instance here.
			String teamLoad = request.getParameter("teamLoad");
			if (teamLoad != null) {
				TeamFromXmlFile teamFromXmlFile = new TeamFromXmlFile(scoreBoardModel, filename, teamLoad);
				teamFromXmlFile.start();
			} else if (start && (null == scoreBoardFromXmlFile)) {
